...
import { deleteComment, updateComment } from "../redux/thunks/comment";
...
const CommentsList = ({
  ...
  const avatar2 = useSelector((state) => state.auth?.user?.avatar);
  ...
  const handleEditComment = (editedContent) => {
    dispatch(updateComment({ commentId, updatedContent: editedContent }));
